...
const Productpage = (props: IIndexPropsg ) => {
  ...
  const [orderOption, setOrderOption] = useState({
    name: props.gift[0].option[0].label,
    price: props.gift[0].option[0].price
  })
      ...
      <section className="container overflow-x-hidden mx-auto text-center flex flex-wrap w-full items-center mt-20" >
        <div className="w-full md:w-2/4">
            ...
            <div className="container ">
                <img className="w-full " src= {props.gift.map(p=>(p.image)).toString()} alt=""  />
           </div>
        </div>
              ...
              p =>{
                return(<>
                  {p.option.map ((i, idx) => {
                    return (
                      <label key={idx}>
                        <input className="boxed-radio"  id="" type="radio" checked={i.label==orderOption ? true: false}  name="Options" value={i.label} 
                          onChange={()=>{changeOrderOption(i.label, i.price)}}    />
                        <span className="option-value-name ">
                          <span >{i.label}</span>
                          <span className="option-serves">{i.serves}</span>
                          <span className="option-price">{i.price}</span>
                        </span>
                      </label>
                    )})}
                </>)
        ...
        <div className="mt-3 mx-2 ">
          <button className="snipcart-add-item text-white uppercase bg-gold p-4 w-full rounded-sm"
            data-item-id={props.gift[0].id}
            data-item-name={props.gift[0].name + "-" }
            data-item-price={Number(orderOption.price)*Number(orderQty)}
            data-item-url={"/product/" + props.gift[0].id}
            data-item-image={props.gift[0].image}>Add to Cart <span>${Number(orderOption.price)*Number(orderQty)}</span></button>
        </div>
        </div>
      </section>
      <Footer />
...
Productpage.getInitialProps =   ({query}) => {
  ...
  let result = giftboxList.filter(obj=>{
